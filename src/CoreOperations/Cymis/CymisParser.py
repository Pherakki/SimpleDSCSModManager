...
def and_operator(arguments, operator_list, flag_list):
    ...
    for item in arguments:
        if type(item) == dict:
            assert len(item) == 1, translate("ModWizards::CYMIS::Logging", "More than one boolean operator in dictionary: {dict_printout}.").format(dict_printout=item)
...
def not_operator(arguments, operator_list, flag_list):
    ...
    if type(arguments) == dict:
        assert len(arguments) == 1, translate("ModWizards::CYMIS::Logging", "More than one boolean operator in dictionary: {dict_printout}.").format(dict_printout=arguments)
...
class CymisInstaller:
    ...
    def install_mod(self):
        if self.log is not None:
            self.log(translate("ModWizards::CYMIS::Logging", "Checking flag table before install..."))
            for flag_name, flag_status in self.flag_table.items():
                self.log(translate("ModWizards::CYMIS::Logging", "{flag_name} is {_bool}.").format(flag_name=flag_name, _bool=flag_status))
        ...
        if log is not None:
            for flag in self.flags:
                for flag_name, _ in flag.get_flag_status().items():
                    self.log(translate("ModWizards::CYMIS::Logging", "Found flag: {flag_type} {flag_name}").format(flag_type=flag.type, flag_name=flag_name))
...
class CymisInstallationStep:
    def __init__(self, path_prefix, step_info, flag_table, log):            
        ...
        if execution_condition is None:
            def check():   
                if log is not None:
                    log(translate("ModWizards::CYMIS::Logging", "Auto-pass: No 'if' statement."))
                return True
            ...
            def check():
                ...
                if log is not None:
                    if result:
                        log(translate("ModWizards::CYMIS::Logging", "Will execute; {execution_condition} is {_bool}.").format(execution_condition=execution_condition, _bool=result))
                    else:
                        log(translate("ModWizards::CYMIS::Logging", "Will not execute; {execution_condition} is {_bool}.").format(execution_condition=execution_condition, _bool=result))
                return result
            ...
            def check():
                ...
                if log is not None:
                    if result:
                        log(translate("ModWizards::CYMIS::Logging", "Will execute; compound statement is {_bool}.").format(_bool=result))
                return result
        else:
            assert 0, translate("ModWizards::CYMIS::Logging", "Unrecognised installation condition type: {data_type}. Should be a string, dict, or not present.").format(data_type=type(execution_condition))
